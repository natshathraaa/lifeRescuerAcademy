          ...
          {data.items.map((course) => {
            ...
            return (
              <motion.div
                ...
                variants={card}
                whileHover={{ y: -6 }}
                className="group relative bg-surface-soft rounded-[24px] p-7 border border-navy-900/[0.06] shadow-card hover:shadow-card-hover hover:bg-white transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center gap-4 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gold-600 font-bold tracking-wider uppercase mt-1">
                        {course.short}
                      </p>
                    </div>
                  </div>
                  ...
                  {course.bullets && course.bullets.length > 0 && (
                    <ul className="mt-5 space-y-2.5 border-t border-navy-900/10 pt-4">
                      {course.bullets.map((bullet, idx) => (
                        <li key={idx} className="flex items-start gap-2.5 text-[13px] font-semibold text-navy-900">
                          <FaCheck className="text-gold-500 shrink-0 mt-0.5" size={12} />
                          <span>{bullet}</span>
                        </li>
                      ))}
                    </ul>
